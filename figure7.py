...
levvar2 = h.variables['lev'][:]
may3 = h.variables['m1'][:,6,::-1,:]
jun3 = h.variables['m2'][:,6,::-1,:]
mj3= (may3+jun3)/2
...
panom_sst=[]
panom_sst=((may2+jun2)/(2))
#calculate the anomaly for the hgt dataset
panom_hgt=[]
panom_hgt=((may3+jun3)/(2*10))
...
plt.title("SEAS5", loc='right', fontsize=12 )
...
m.drawparallels(parallels,labels=[1,0,0,0],fontsize=10,linewidth=0.01)
m.drawmeridians(meridians,labels=[0,0,0,1],fontsize=10,linewidth=0.01)
map2=m.contourf(x,y,satiN_mean,np.arange(-0.7,0.8,0.1),cmap=plt.cm.RdBu_r,extend='both')
...
plt.title("SEAS5", loc='right', fontsize=14 )
plt.tight_layout()
cbar_ax2 = fig.add_axes([0.2, 0.08, 0.6, 0.03])  # [left, bottom, width, height]
cbar2 = fig.colorbar(map2, cax=cbar_ax2, orientation='horizontal', shrink=0.8, pad=0.8)
cbar2.ax.tick_params(labelsize=14) 
plt.savefig('figure7adbecf.png', dpi = 300)
plt.show()
